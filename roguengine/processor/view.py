...
class ViewProcessor(Processor):
    ...
    def process(self):
        ...
        entities = self.world.get_components(PositionComponent, SpriteComponent)
        ...
        if not inside_room:
            for ent, (pos, _) in entities:
                ...
                if not (px - 1 <= x <= px + 1 and py - 1 <= y <= py + 1):
                    if self.world.has_component(ent, DungeonResidentComponent) and self.world.has_component(ent, VisibleComponent):
